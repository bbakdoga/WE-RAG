...
export default function Skills() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Tabs */}
      <div className="tabs">
        <button className={`tab-item ${activeTab === 'quizzes' ? 'active' : ''}`} onClick={() => setActiveTab('quizzes')}>Skill Quizzes</button>
        <button className={`tab-item ${activeTab === 'badges' ? 'active' : ''}`} onClick={() => setActiveTab('badges')}>Badge Gallery</button>
        <button className={`tab-item ${activeTab === 'points' ? 'active' : ''}`} onClick={() => setActiveTab('points')}>How to Earn Points</button>
      </div>
        ...
        <>
          <div className="filter-pills" style={{ marginBottom: 'var(--space-6)' }}>
            {badgeCategories.map(cat => (
              <button key={cat} className={`chip ${badgeFilter === cat ? 'active' : ''}`} onClick={() => setBadgeFilter(cat)}>
                {cat}
              </button>
            ))}
          </div>
          <div className="grid-3">
            {filteredBadges.map(badge => (
              <div key={badge.id} className={`card ${badge.earned ? 'card-elevated' : ''}`} style={{ padding: 'var(--space-5)', textAlign: 'center', opacity: badge.earned ? 1 : 0.7, position: 'relative' }}>
